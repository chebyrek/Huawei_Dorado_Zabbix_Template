...
# LOG_FILENAME = "/tmp/huawei_state1.log"
...
def get_performance_stat(headers, api_cookies, api_ip, api_port, device_id, resource, check_id):
	# получить статистику по производительности
	perf_url = f"https://{api_ip}:{api_port}/deviceManager/rest/{device_id}/performace_statistic/cur_statistic_data?CMO_STATISTIC_UUID={resource}&CMO_STATISTIC_DATA_ID_LIST={check_id}"
	perf_info = requests.get(perf_url, verify=False, cookies = api_cookies, headers = headers)
	perf_info = json.loads(perf_info.content.decode('cp1251'))
	return perf_info['data'][0]['CMO_STATISTIC_DATA_LIST']
		...
		for resource in list_resources:
			...
			if ['fc_port','sas_port', 'eth_port'].count(resource) == 1:
				...
				for one_res in resource_info['data']:
					...
					key_info = "info.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))
					info = f"{one_res['MANUFACTURER']} {one_res['MODEL']} SN:{one_res['SERIALNUMBER']} BC:{one_res['barcode']}"
					state_resources.append("%s %s %s %s" % (storage_name, key_info, timestampnow, info))

					key_temp = "temperature.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))
					state_resources.append("%s %s %s %s" % (storage_name, key_temp, timestampnow, one_res['TEMPERATURE']))

					key_runtime = "working_days.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))
					state_resources.append("%s %s %s %s" % (storage_name, key_runtime, timestampnow, one_res['RUNTIME']))

					key_healthmark = "health_mark.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))
					state_resources.append("%s %s %s %s" % (storage_name, key_healthmark, timestampnow, one_res['HEALTHMARK']))

					key_remainlife = "remain_life.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))
					state_resources.append("%s %s %s %s" % (storage_name, key_remainlife, timestampnow, one_res['REMAINLIFE']))
			elif ['enclosure'].count(resource) == 1:
				for one_res in resource_info['data']:
					key_health = "health.{0}.[{1}]".format(resource, one_res['NAME'].replace(' ','_'))  # Создаем ключ для каждого объекта и заменяем пробелы
					key_status = "running.{0}.[{1}]".format(resource, one_res['NAME'].replace(' ','_'))
					state_resources.append("%s %s %s %s" % (storage_name, key_health, timestampnow, one_res['HEALTHSTATUS']))
					state_resources.append("%s %s %s %s" % (storage_name, key_status, timestampnow, one_res['RUNNINGSTATUS']))

					key_mac = "mac.{0}.[{1}]".format(resource, one_res['NAME'].replace(' ','_'))
					state_resources.append("%s %s %s %s" % (storage_name, key_mac, timestampnow, one_res['MACADDRESS']))
					key_sn = "sn.{0}.[{1}]".format(resource, one_res['NAME'].replace(' ','_'))
					state_resources.append("%s %s %s %s" % (storage_name, key_sn, timestampnow, one_res['SERIALNUM']))
					key_temp = "temperature.{0}.[{1}]".format(resource, one_res['NAME'].replace(' ','_'))
					state_resources.append("%s %s %s %s" % (storage_name, key_temp, timestampnow, one_res['TEMPERATURE']))
			elif ['controller'].count(resource) == 1:
				for one_res in resource_info['data']:
					key_health = "health.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))  # Создаем ключ для каждого объекта и заменяем пробелы
					key_status = "running.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))
					state_resources.append("%s %s %s %s" % (storage_name, key_health, timestampnow, one_res['HEALTHSTATUS']))
					...
					key_cpu = "cpu.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))
					state_resources.append("%s %s %s %s" % (storage_name, key_cpu, timestampnow, one_res['CPUUSAGE']))

					key_mem = "mem.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))
					state_resources.append("%s %s %s %s" % (storage_name, key_mem, timestampnow, one_res['MEMORYUSAGE']))

					key_io = "io.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))
					io = get_performance_stat(headers, api_cookies, api_ip, api_port, device_id, f"{one_res['TYPE']}:{one_res['ID']}", "22")
					state_resources.append("%s %s %s %s" % (storage_name, key_io, timestampnow, io))
			else:
				for one_res in resource_info['data']:
					key_health = "health.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))  # Создаем ключ для каждого объекта и заменяем пробелы
					key_status = "running.{0}.[{1}]".format(resource, one_res['LOCATION'].replace(' ','_'))
